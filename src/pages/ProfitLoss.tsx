...
export default function ProfitLoss() {
  ...
  const getRange = (p: string, offset = 0) => {
    const ref = p === "month" ? subMonths(now, offset)
      : p === "quarter" ? subQuarters(now, offset)
      : subYears(now, offset);
  ...
  useEffect(() => {
    ...
    fetchAccountBalances(start, end).then((d) => {
      ...
      if (compare) {
        ...
        fetchAccountBalances(prev.start, prev.end).then((pd) => {
          setPrevLines(pd);
          setLoading(false);
        });
      } else {
  ...
  const grouped = useMemo(() => {
    ...
    expenses.forEach((e) => {
      const key = e.accountSubtype ?? "overig";
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              {compare && <TableHead className="text-right">Vorige periode</TableHead>}
            </TableRow>
          </TableHeader>
          <TableBody>
                ...
                <TableRow key={l.code}>
                  ...
                  {compare && (
                    <TableCell className="text-right tabular-nums text-muted-foreground">
                      {prevLine ? fmt(Math.abs(prevLine.balance)) : "—"}
                    </TableCell>
                  )}
                </TableRow>
              );
            ...
            <TableRow className="font-semibold border-t-2">
              <TableCell />
              <TableCell>Totaal {title.toLowerCase()}</TableCell>
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Label htmlFor="compare" className="text-sm text-muted-foreground">Vergelijken</Label>
          </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base font-medium">Kosten</CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  ...
                  <TableRow className="font-semibold border-t-2">
                    ...
                    <TableCell className="text-right tabular-nums">{fmt(totalExpenses)}</TableCell>
                    {compare && (
                      <TableCell className="text-right tabular-nums text-muted-foreground">
                        {fmt(prevTotalExpenses)}
                      </TableCell>
                    )}
                  </TableRow>
                </TableBody>
          ...
          <Card className={result >= 0 ? "border-primary/30" : "border-destructive/30"}>
            <CardContent className="flex items-center justify-between py-5">
              ...
              <div className="text-right">
                <span className={`text-xl font-bold tabular-nums ${result >= 0 ? "text-primary" : "text-destructive"}`}>
                  {fmt(result)}
                </span>
                {compare && (
                  <span className="ml-4 text-sm text-muted-foreground tabular-nums">{fmt(prevResult)}</span>
                )}
              </div>
            </CardContent>
          </Card>
