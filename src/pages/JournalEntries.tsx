...
export default function JournalEntries() {
  ...
  const handleExport = () => {
    ...
    const rows = entries.map((e) =>
      `${e.entry_number},"${e.date}","${(e.description ?? "").replace(/"/g, '""')}",${e.status},${e.source_type ?? "system"},${e.ai_confidence != null ? Math.round(e.ai_confidence * 100) + "%" : ""}`
    ).join("\n");
    const blob = new Blob([header + rows], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `journaalposten_${filters.dateFrom}_${filters.dateTo}.csv`;
    a.click();
  ...
  return (
    <div className="space-y-4">
      ...
      <JournalFilters filters={filters} onChange={setFilters} onExport={handleExport} />

      <JournalTable entries={entries} isLoading={isLoading} onRowClick={(id) => setDetailId(id)} />
