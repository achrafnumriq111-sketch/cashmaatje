        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">Aantal posten</p>
                <p className="text-xl font-semibold tabular-nums">{expenses.filter(e => e.is_active).length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">Jaarlijks totaal</p>
                <p className="text-xl font-semibold tabular-nums">{fmt(annualTotal)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-semibold tabular-nums">{fmt(annualTotal / 12)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Select value={editing.frequency} onValueChange={(v) => setEditing({ ...editing, frequency: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {FREQUENCIES.map((f) => <SelectItem key={f} value={f}>{frequencyLabels[f]}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="space-y-2">
              {expenses.map((e) => (
                <div key={e.id} className={`flex items-center justify-between rounded-lg border p-3 ${e.is_active ? "" : "opacity-50"}`}>
                  <div className="flex items-center gap-3">
                    <div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => e.id && remove(e.id)}>
