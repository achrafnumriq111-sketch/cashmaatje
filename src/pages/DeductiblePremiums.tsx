...
const typeLabels: Record<string, string> = { aov: "AOV (Arbeidsongeschiktheid)", lijfrente: "Lijfrente / Pensioen" };

const emptyPremium = (year: number): DeductiblePremium => ({
  year, premium_type: "aov", name: "", amount: 0, is_active: true, notes: null,
});
...
export default function DeductiblePremiumsPage() {
  ...
  const handleSave = async () => {
    ...
    await save(editing); setEditing(null); toast.success("Premie opgeslagen");
  };
    ...
    <motion.div variants={pageTransition} initial="initial" animate="animate" exit="exit" className="space-y-6">
      <motion.div variants={cardVariant} className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button size="sm" onClick={() => setEditing(emptyPremium(year))}><Plus className="mr-1.5 h-4 w-4" /> Toevoegen</Button>
        </div>
      </motion.div>

      <motion.div variants={staggerContainer} initial="initial" animate="animate" className="grid gap-4 sm:grid-cols-2">
        {[
          { icon: <Shield className="h-5 w-5 text-primary" />, label: "AOV totaal", value: fmt(premiums.filter(p => p.is_active && p.premium_type === "aov").reduce((s, p) => s + p.amount, 0)) },
          { icon: <Heart className="h-5 w-5 text-primary" />, label: "Lijfrente totaal", value: fmt(premiums.filter(p => p.is_active && p.premium_type === "lijfrente").reduce((s, p) => s + p.amount, 0)) },
        ].map((kpi, i) => (
      ...
      {editing && (
        <motion.div variants={cardVariant} initial="initial" animate="animate">
          <Card className="arcory-glass border-primary/30">
            <CardHeader className="pb-3"><CardTitle className="text-base">{editing.id ? "Bewerk premie" : "Nieuwe premie"}</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2"><Label>Naam</Label><Input value={editing.name} onChange={(e) => setEditing({ ...editing, name: e.target.value })} placeholder="bijv. AOV bij Movir" /></div>
                <div className="space-y-2">
                  <Label>Type</Label>
                  <Select value={editing.premium_type} onValueChange={(v) => setEditing({ ...editing, premium_type: v })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent><SelectItem value="aov">AOV</SelectItem><SelectItem value="lijfrente">Lijfrente</SelectItem></SelectContent>
                  </Select>
                </div>
                <div className="space-y-2"><Label>Jaarbedrag (€)</Label><Input type="number" value={editing.amount} onChange={(e) => setEditing({ ...editing, amount: parseFloat(e.target.value) || 0 })} /></div>
              </div>
      ...
      <motion.div variants={cardVariant}>
        <Card className="arcory-glass">
          <CardHeader className="pb-3"><CardTitle className="text-base">Premies {year}</CardTitle></CardHeader>
          <CardContent>
            {loading ? <p className="text-sm text-muted-foreground">Laden...</p> : premiums.length === 0 ? <p className="text-sm text-muted-foreground">Geen premies gevonden voor {year}.</p> : (
              <div className="space-y-2">
                {premiums.map((p) => (
                  <div key={p.id} className={`flex items-center justify-between rounded-lg border border-border/50 p-3 ${p.is_active ? "" : "opacity-50"}`}>
                    <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setEditing(p)}><Save className="h-3.5 w-3.5" /></Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => p.id && remove(p.id)}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </div>
                ))}
              </div>
