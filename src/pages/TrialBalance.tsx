...
export default function TrialBalance() {
  ...
  const filtered = useMemo(
    () => (typeFilter === "all" ? lines : lines.filter((l) => l.accountType === typeFilter)).filter(
      (l) => Math.abs(l.debitTotal) > 0.005 || Math.abs(l.creditTotal) > 0.005
    ),
    [lines, typeFilter]
  );
              ...
              <TableBody>
                {filtered.map((l) => (
                  <TableRow key={l.code}>
                    ...
                    <TableCell className={`text-right tabular-nums font-medium ${l.balance > 0 ? "" : l.balance < 0 ? "text-destructive" : "text-muted-foreground"}`}>
                      {fmt(l.balance)}
                    </TableCell>
                  </TableRow>
                ))}
              ...
              <TableFooter>
                <TableRow className="font-semibold">
                  ...
                  <TableCell className="flex items-center gap-2">
                    Totaal
                    {!balanced && (
                      <Badge variant="destructive" className="text-[10px]">Niet in balans</Badge>
                    )}
                  </TableCell>
