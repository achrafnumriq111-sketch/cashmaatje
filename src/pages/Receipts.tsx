...
import { LayoutGrid, List, Camera, Building2, Receipt } from "lucide-react";
...
export default function Receipts() {
  ...
  const suppliersWithMultiple = receipts.filter((d) => d.extracted_supplier_name?.trim()).length;
    ...
    <motion.div variants={pageTransition} initial="initial" animate="animate" exit="exit" className="space-y-4 sm:space-y-6">
      <motion.div variants={cardVariant} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Scan of upload bonnetjes — automatisch gegroepeerd per leverancier.</p>
        </div>
